  ...
  return (
    <main style={{ minHeight: '100vh', paddingTop: '8rem', paddingBottom: '6rem' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 2rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 300, marginBottom: '3rem', fontFamily: 'Cormorant Garamond' }}>
        ...
        <div style={{ display: 'flex', gap: '0', marginBottom: '3rem', borderBottom: '1px solid var(--brown-light)' }}>
          {Object.keys(menu).map(cat => (
            <button key={cat} onClick={() => setActive(cat)} style={{
              ...
              fontFamily: 'Jost', transition: 'all 0.3s', marginBottom: '-1px',
            }}>
              {cat}
            </button>
          ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {menu[active].map((item, i) => (
            <div key={item.name} style={{
              ...
              <div>
                <h3 style={{ fontSize: '1.2rem', fontWeight: 400, marginBottom: '0.4rem', color: 'var(--cream)', fontFamily: 'Cormorant Garamond' }}>{item.name}</h3>
                <p style={{ color: 'var(--text-light)', fontSize: '0.85rem', lineHeight: 1.6 }}>{item.desc}</p>
              </div>
              <span style={{ color: 'var(--gold)', fontFamily: 'Cormorant Garamond', fontSize: '1.1rem', whiteSpace: 'nowrap', marginLeft: '2rem' }}>{item.price}</span>
            </div>
          ))}
        </div>
      </div>
